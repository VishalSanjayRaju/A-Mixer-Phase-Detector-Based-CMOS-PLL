...
The Schematic diagram of the PLL is as shown in the above image. We can observe the right side of the schematic we have voltage controlled oscillator which is biased by the current mirror circuit. Initial current is around 50uA. The feedback is given to the mixer circuit which is taking gthe 2 input signals i.e reference voltage and feedback voltage. This acts as a phase frequency detector in PLL. 
...
The transient analysis of the PLL circuit is as shown in the above plot. The plot of output signal and reference signal is as shown. 
...
The plot of the frequency spectrum is as shown in the above plot. As we can see we have the highest magnitude when the frequency is around 15.8GHz. The plot we have got by taking the samples from the cadence EDA tool and then doing FFT for the samples. 
...
The above table shows the power consumption when there is a variation in process and temperature.
...
The above plot shows the change in the output frequency when the supply voltage changes. As the supply voltage (VDD) increases then the output frequency of the PLL also increases.
